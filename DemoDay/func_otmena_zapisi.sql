...
--Отмена записи
create or replace procedure LEBEDEV_MA.cancel_sign_up(
    p_id_talon number
)
as
begin
    update LEBEDEV_MA.JOURNAL jr
    set jr.ID_STATUS = (select st.ID_Status from LEBEDEV_MA.STATUS st where st.STATUS_NAME = 'Отменено')
...
begin
    ...
    if (not v_error_status) then
        LEBEDEV_MA.cancel_sign_up(p_id_talon => p_id_talon);
