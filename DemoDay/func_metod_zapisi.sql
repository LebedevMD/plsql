...
begin
    ...
    where jr.ID_PATIENT = p_id_patient and jr.ID_TALON = p_id_talon;
    if (v_count != 0) then
        return true;
    else
        return false;
...
--Запись
create or replace procedure LEBEDEV_MA.registration_for_the_talon(
    p_id_patient number,
    p_id_talon number
)
as
begin
    ...
    values
        (p_id_patient, p_id_talon,
         (select st.ID_Status from LEBEDEV_MA.STATUS st where st.STATUS_NAME = 'Подтверждено'));
end;
...
begin
    ...
    if (not v_error_status) then
        LEBEDEV_MA.registration_for_the_talon(p_id_patient => p_id_patient, p_id_talon => p_id_talon);
