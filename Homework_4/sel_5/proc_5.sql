...
begin
    ...
    LEBEDEV_MA.show_all_talon_by_id_doctor(p_id_doc => v_id_doc, out_cursor => v_cursor_talon);
    fetch v_cursor_talon into v_doctors;
    if(v_cursor_talon%found) then
        loop
            DBMS_OUTPUT.PUT_LINE(v_doctors.Name || '; ' ||
                                 to_char(v_doctors.StartDate, 'dd.mm.yy hh24:mi') ||
                                 ' - ' || to_char(v_doctors.EndDate, 'dd.mm.yy hh24:mi'));
            fetch v_cursor_talon into v_doctors;
