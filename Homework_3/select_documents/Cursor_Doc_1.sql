...
declare
    cursor cursor_1(p_id_pat number) is (
        ...
        where
              ((p_id_pat is not null and pat.ID_PATIENT = p_id_pat)
                  or (p_id_pat is null and pat.ID_PATIENT is not null)));
...
begin
    ...
    loop
        ...
        DBMS_OUTPUT.PUT_LINE(
            v_record_docs.Surname ||
            ' ' ||
            v_record_docs.Name ||
            '; ' ||
            v_record_docs.DocName ||
            ': ' ||
            v_record_docs.DocValue);
    end loop;
    close cursor_1;
end;