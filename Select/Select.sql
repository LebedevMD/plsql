...
select sp.NAME as Специальность from LEBEDEV_MA.HOSPITALS h join LEBEDEV_MA.DOCTORS d on d.ID_HOSPITAL = h.ID_HOSPITAL
    join LEBEDEV_MA.SPECIALITY sp on d.ID_SPECIALITY = sp.ID_SPECIALITY where d.DELETED is null and sp.DELETED is null and h.DELETED is null;
...
select h.ID_HOSPITAL as Айди_филиала, mo.Name as Мед_Организация, sp.Name as Специальность
...
where sp.NAME = 'Хирург' order by h.ID_TYPE desc;
...
where d.ID_HOSPITAL = 1 order by q.ID_QUALIFICATION;
